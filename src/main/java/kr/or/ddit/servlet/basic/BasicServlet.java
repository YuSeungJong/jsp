...
public class BasicServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		logger.debug("basicServet.doGet()");
		
		// 만약에 log level을 debug보다 높은 레벨로 설정할 경우
		// 로그를 생성하지는 않지만 메소드 인자인 문자열 + 문자열 ==> 문자열 결하 연산은 발생하다
		
		//if(설정로그레벨 <= debug) {
			logger.debug("basicServet.doGet() userid parameter : " + req.getParameter("userid"));
		//}
		logger.debug("basicServet.doGet() userid parameter : {} {}", 
		...
		// init 메소드는 해당 서블릿에서 사용자 자원을 초기화 할 때 사용
		...
		//								서블릿에서 처리(JspServlet)
		// localhost/doc/20201223.txt	정적자료 ==> 다음주
